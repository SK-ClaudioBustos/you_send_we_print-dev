...
function turnaround_list(measure_type) {
    ...
    let turnaround_calculate =  $("#turnaround_calculate").val();
    //console.log(turnaround_calculate);
    let measure_unit = $(".measure_unit:checked").val();
    let total = 0;
    let width = 0;
    let height = 0;
    if (measure_type == "fixed" || turnaround_calculate == "units" ) { // si son remeras (fixed) o en el caso de "units" calcula por totales, no sqft
       // per unit
        total = quantity;
    } else {
        // per sqft
    ...
    total_turnarounds = parseInt(total); // convert to ingeger for to exact range.
    if (turnarounds) {
        //} && total) {
        for (var turn in turnarounds) {
            if (
                total_turnarounds >= parseFloat(turnarounds[turn]["min"]) &&
                (total_turnarounds <= parseFloat(turnarounds[turn]["max"]) ||
                    parseFloat(turnarounds[turn]["max"] == "n"))
            ) {
                break;
            }
        }
       // console.log(turnarounds[turn]);
        if (turnarounds[turn]) {
            var new_range =
...
function init_product_form(logged, measure_type) {
    ...
    $(function () {
        ...
        $("#accesory1, #accesory2, #accesory3").select2({
            ...
            templateResult: function (state) {
                ...
                console.log("corte:"+corte)    
                 if (filtro != undefined && $opt.text())
                  if (!$opt.text().includes(filtro) && !$opt.text().includes("Needed") && !$opt.text().includes("needed")) {
                    return null;
                 }
                   return state.text;
             }
     ...
     /* $('.select2').on("select2:selecting", function(e) { 
          //corte = e.params.args.data.filter.split("/");   // what you would like to happen
           //alert(corte[1]);
          // $("#accesory1, #accesory2, #accesory3").empty();
           if (corte[1]!= undefined )
                $("#accesory1, #accesory2, #accesory3").select2("val", "");  
           return e.value;
        });
        */
        ...
        $('.select2').on('select2:select', function (e) {
             ...
             isMaterial = "";
             if (e.params.data) {
             if (e.params.data._resultId.includes("material"))
                 isMaterial = e.params.data._resultId.includes("material");
             if (e.params.data.element.className)
                corte = e.params.data.element.className;   // what you would like to happen
              if (isMaterial){
     ...
     $("#accesory1").on("change", function () {
          if($("#accesory1").val() === null || $('#select2-accesory1-container').attr('title')=="I will cut it myself / No cut needed"|| $('#select2-accesory1-container').attr('title')=="No Accessories Needed")
          { $("#accesory1_qty").val("");
            $("#accesory1_qty").attr("reference","");
            $("#accesory1_qty").attr("readonly","true");
          }
          else
          {
          ...
          if (($("#accesory1_qty").val() > 0 || $("#accesory1_qty").val() == "" ) &&  $('#select2-accesory1-container').attr('title')!="I will cut it myself / No cut needed" ) {
       ...
       $("#accesory2").on("change", function () {
        ...
        if($("#accesory2").val() === null || $('#select2-accesory2-container').attr('title')=="I will cut it myself / No cut needed"|| $('#select2-accesory2-container').attr('title')=="No Accessories Needed")
          {$("#accesory2_qty").val("");
            $("#accesory2_qty").attr("reference","");
            $("#accesory2_qty").attr("readonly","true");
          }
          else
          {
        ...
        if (($("#accesory2_qty").val() > 0 || $("#accesory2_qty").val() == "" ) &&  $('#select2-accesory2-container').attr('title')!="I will cut it myself / No cut needed") {
       ...
       $("#accesory3").on("change", function () {
        ...
        if($("#accesory3").val() === null || $('#select2-accesory3-container').attr('title')=="I will cut it myself / No cut needed"|| $('#select2-accesory3-container').attr('title')=="No Accessories Needed")
          {$("#accesory3_qty").val("");
            $("#accesory3_qty").attr("reference","");
            $("#accesory3_qty").attr("readonly","true");
        ...
        if (($("#accesory3_qty").val() > 0 || $("#accesory3_qty").val() == "" ) &&  $('#select2-accesory3-container').attr('title')!="I will cut it myself / No cut needed") {
        ...
        for (let item_list in maximums) {
            ...
            //console.log(maximums[item_list]);
