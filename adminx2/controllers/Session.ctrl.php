...
//			unset($_SESSION['session_target']);
	...
	protected function run_logout($args) {
		if ($user_id = $_SESSION[md5($_SERVER['REMOTE_ADDR'])]) {
			...
			$session = new Session();
			$session->delete_by_user($user_id);
		}

		// invalidate 'remember' cookies
		if ($this->get_cookie('k') !== false) {
		   $this->set_cookie('k', '', time()-60*60*24*30);
		}
	...
	protected function run_login($args, $captcha = false) {
		if ($this->get_input('action', '') == 'login') {
			...
			if ($captcha) {
				...
				$publickey = '6Lc9AeoSAAAAAGaMc6fIIBGDj5is4LzG3PiU2WTh';
				// $privatekey is in _Main.ctrl.php
			}
		...
		if ($this->check_form_token('remind')) {
			...
			if (sizeof($error)) {
				...
				if ($user->retrieve_by_email($data['email'])) {
					// set activation key
					$act_key = $this->get_uid($user_id);
					$act_limit = $this->date_modify(date('Y-m-d H:i:s'), '+1 day', false, 'Y-m-d H:i:s');
					$act_limit_txt = $this->date_format($act_limit, '-3:00', $this->app->datetime_format);

					$user->set_activation_key($act_key);
					$user->set_activation_limit($act_limit);
					$user->update();

					$views = $this->tpl->get_view('_emails/remind', array(
					...
					$this->notify(false, array($data['email'] => $user->get_username()), $views['subject'], $views['body']);
				}
	...
	protected function run_remember($args) {
		...
		$session = new Session();
		$session->retrieve_by_key($session_key);
...
//					$session->delete_by_user($user_id);
					...
					$session->set_time_limit($this->date_modify(date('Y-m-d H:i:s'), '+1 month', false, 'Y-m-d H:i:s'));
					$session->update();
	...
	private function reset_save($object, $activation_key) {
		...
		if ($activation_key != $data['activation_key']) {
			...
			$error_fields = $this->validate_data($data, array(
					...
					'rpassword'	=> array('string', false, $this->app->user_password_len_min, $this->app->user_password_len_max),
				));
	...
	private function login_save() {
		...
		if ($valid) {
			...
			if ($this->get_cookie('k') !== false) {
				$session = new Session();
				$session->delete_by_key($this->get_cookie('k'));

				$this->set_cookie('session', '', time()-60*60*24*30);
			}
			...
			$data = array(
					...
					'recaptcha_challenge'	=> $this->get_input('recaptcha_challenge_field', '', true),
					'recaptcha_response'	=> $this->get_input('recaptcha_response_field', '', true),
				);

			$error_fields = $this->validate_data($data, array(
					'username' 		=> array('string', false, 1, 50),
					'user_password' => array('string', false, 1, 50),
				));
			$error = $this->missing_fields($error_fields);
			...
			if ($req_captcha) {
				...
				if (!isset($_POST['recaptcha_challenge_field'])) {
					...
					$privatekey = '6Lc9AeoSAAAAAEFYszAGK_HP9mPEpqIe1miXuQpB';
					// $publickey is in Home.ctrl.php
					$resp = recaptcha_check_answer($privatekey, $_SERVER["REMOTE_ADDR"], $data['recaptcha_challenge'], $data['recaptcha_response']);
	...
	private function get_uid($user_id) {
	    ...
	    return $id_part . $this->get_token(28);
	}
