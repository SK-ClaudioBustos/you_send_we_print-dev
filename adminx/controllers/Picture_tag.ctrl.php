 ...
 *
 * -------------------------------------------------------
 * CLASSNAME:        Picture_tagCtrl
 * GENERATION DATE:  2018-06-20
 * -------------------------------------------------------
  *
 */

class Picture_tagCtrl extends CustomCtrl {
	protected $mod = 'Picture_tag';
	protected $class = 'Picture_tag';
	...
	protected function run_single($object, $args = array()) {
		// temp
		$page_args = $categorys = array();

		/*
		$categorys = new Category();
		$categorys->set_paging(1, 0, "`category` ASC");
	...
	protected function run_export($args = []) {
		...
		$header = array(
				$this->lng->text('picture_tag:picture_tag'),
				$this->lng->text('picture_tag:category_id'),
			);
