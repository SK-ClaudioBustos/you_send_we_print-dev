...
//			if($t.p.subGrid === true || $t.p.treeGrid === true || $t.p.cellEdit === true || $t.p.sortable || $t.p.scroll )
			if($t.p.subGrid === true || $t.p.cellEdit === true || $t.p.scroll )
			{
				return;
			}
			if($t.p.rownumbers) { i++; }
	...
	expandRow: function (record){
		this.each(function(){
			...
			$(childern).each(function(){
				...
				if( $t.p.frozenColumns === true ) {
					var rcf = $("#" + $t.p.idPrefix + $.jgrid.jqID(id), $t.grid.fbDiv);
					rcf.css("display","");
				}
		...
		return this.each(function(){
			...
			if(!rc[expanded]) {
				...
				if( $(this).jqGrid("isNodeLoaded",this.p.data[position]) ) {
					...
					var $t = this;
					if( $t.p.frozenColumns === true ) {
						var frz = "#" + $t.p.id+"_frozen";
						var rcf = $("#" + this.p.idPrefix + $.jgrid.jqID(id), frz)[0];
						$("div.treeclick", rcf).removeClass(this.p.treeIcons.plus+" tree-plus").addClass(this.p.treeIcons.minus+" tree-minus");
					}
...
var ts= this;					
$(ts).triggerHandler("jqGridTreeNodeExpand");
	...
	collapseNode : function(rc) {
		return this.each(function(){
			...
			if(rc[expanded]) {
				...
				var $t = this;
				if( $t.p.frozenColumns === true ) {
					var rcf = $("#" + this.p.idPrefix + $.jgrid.jqID(id), $t.grid.fbDiv);
					$("div.treeclick",rcf).removeClass(this.p.treeIcons.minus+" tree-minus").addClass(this.p.treeIcons.plus+" tree-plus");
				}
			}
		});
	},
