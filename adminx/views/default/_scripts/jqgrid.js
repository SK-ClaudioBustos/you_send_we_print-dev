...
var render_check = function (cellvalue, options, rowObject) {
...
var set_grid_height = function (grid) {
	let height = $(window).height() - grid.offset().top - 90;
	height = (height < 400) ? 400 : height;
	let width = $('.content').width();

	grid.jqGrid('setGridWidth', width);
	grid.jqGrid('setGridHeight', height); // header is outside setGridHeight
};
...
function init_grid_tools() {
	...
	$('#jqg-list').on('click', '.tool_del', function() {
		...
		bootbox.confirm(lng_text('tool:confirm_del'), function(result) {
			if (result) {
