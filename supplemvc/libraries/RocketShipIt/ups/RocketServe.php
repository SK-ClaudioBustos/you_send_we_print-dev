...
require_once("../rocketshipit/php/RocketShip.php");
...
if (!in_array($request['type'], $valid_classes)) {
    ...
    echo '{"error": "Invalid or unsupported class type for this demo.  This may be supported in RocketShipIt but excluded from the demo."}';
    exit;
}
...
try {
    ...
    if (in_array('packages', array_keys($request))) {
        foreach ($packages as $package) {
            $p = new RocketShipPackage($carrier);
...
$logger->log('Response: ', $response);
$logger->log('Response: ', html_entity_decode($obj->core->debug()));
