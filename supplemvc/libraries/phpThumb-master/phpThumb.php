...
function SendSaveAsFileHeaderIfNeeded($phpThumb, $PHPTHUMB_CONFIG,$getimagesize=false) {
	if (headers_sent()) {
		return false;
	}
	//global $phpThumb;
...
//var_dump($phpThumb);
	...
	//SendSaveAsFileHeaderIfNeeded($getimagesize);
		SendSaveAsFileHeaderIfNeeded($phpThumb, $PHPTHUMB_CONFIG,$getimagesize);
...
while ($CanPassThroughDirectly && $phpThumb->src) {
	...
	if ($phpThumb->getimagesizeinfo = @getimagesize($SourceFilename)) {
		...
		if ($phpThumb->config_disable_onlycreateable_passthru || (function_exists($theImageCreateFunction) && ($dummyImage = @$theImageCreateFunction($SourceFilename)))) {
			...
			SendSaveAsFileHeaderIfNeeded($phpThumb, $PHPTHUMB_CONFIG,$phpThumb->getimagesizeinfo);
